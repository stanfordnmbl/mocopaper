...
\begin{align}
    \begin{aligned}
        \mbox{minimize} \quad
         & \sum_j w_j J_{c,j}(t_0, t_f, y_0, y_n, x_{0}, x_{n}, \lambda_0, \mathrlap{\lambda_n, p, S_{c,j})
         + w_{\lambda} \sum_{i=1}^{n} \textrm{simpson}_i(\|\lambda\|_2^2)}  \\
         & \quad\quad\quad S_{c,j} = \sum_{i=1}^{n} \textrm{simpson}_i(s_{c,j}(t, y, x, \lambda, p)) \\
        \mbox{subject to} \quad
         & \bar{q}_i = \textrm{hermite}_i(q, u) + G(\bar{q}_i, p)^T \bar{\gamma}_i && i = 1, \ldots, n \\
         & q_i = q_{i-1} + \textrm{simpson}_i(u) && i = 1, \ldots, n \\
         & \bar{u}_i = \textrm{hermite}_i(u, f_{\textrm{mb}}(t, y, x, \lambda, p)) && i = 1, \ldots, n \\
         ...
         & V_{L,k} \leq V_k(t_0, t_f, y_0, y_f, x_{0}, x_{f}, \lambda_0, \mathrlap{\lambda_f, p, S_{e,k}) \leq V_{U,k}} \\
        & \quad\quad\quad S_{e,j} = \sum_{i=1}^{n} \textrm{simpson}_i(s_{e,j}(t, y, x, \lambda, p)) \\
        & \quad\quad\quad k = 1, \ldots, K \\
        & g_{L} \leq g(t_i, y_i, x_{i}, p) \leq g_{U}  && i = 0, \ldots, n\\
         \mbox{with respect to} \quad
         & t_0 \in [t_{0,L}, t_{0,U}] \\
...
\begin{table}[!ht]
    \begin{adjustwidth}{-2.25in}{0in} % Comment out/remove adjustwidth environment if table fits in text column.
        \centering
        \caption{
        {\bf Moco’s solvers. (opt) indicates an optional dependency.}}
...
Computed Muscle Control (CMC), MocoTrack, and MocoInverse produced muscle activation that was consistent with electromyography. MocoInverse and CMC yielded similar muscle activation (root-mean-square difference of 0.091), while MocoTrack produced higher muscle activation than CMC and MocoInverse for some muscles (e.g., rectus femoris, biceps femoris, vastus lateralis, and tibialis anterior). The difference between required net joint moments and muscle-generated net joint moments (termed “reserve” moments in OpenSim) had a maximum value of 0.12 N-m for MocoTrack and 7.8 N-m for MocoInverse~\cite{Hicks:2015bo}. CMC solved in 2.5 minutes, MocoTrack solved in 2.8 hours, and MocoInverse solved in 7.7 minutes.
