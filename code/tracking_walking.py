...
class MotionTrackingWalking(MocoPaperResult):
    def __init__(self):
        ...
        self.tracking_weights = [1] # 1, 0.5, 0.001, 0]
        self.effort_weights = [0] # 0.001, 0.5, 1, 1]
        self.cmap = 'nipy_spectral'
        self.cmap_indices = [0.2, 0.35, 0.5, 0.75, 0.9]
    ...
    def create_model_processor(self, root_dir, for_inverse=False):
        ...
        add_reserve(model, 'lumbar_extension', 0.1, 50)
        add_reserve(model, 'lumbar_bending', 0.1, 50)
        add_reserve(model, 'lumbar_rotation', 0.1, 50)
        add_reserve(model, 'pelvis_tilt', 0.1, reserves_max)
        add_reserve(model, 'pelvis_list', 0.1, reserves_max)
        add_reserve(model, 'pelvis_rotation', 0.1, reserves_max)
        add_reserve(model, 'pelvis_tx', 0.1, reserves_max)
        add_reserve(model, 'pelvis_ty', 0.1, reserves_max)
        add_reserve(model, 'pelvis_tz', 0.1, reserves_max)
    ...
    def generate_results(self, root_dir, args):
        ...
        # solution = osim.MocoTrajectory()
        solution = osim.MocoTrajectory(
            self.get_solution_path(root_dir, self.tracking_weights[0],
                  self.effort_weights[0]))
