...
Including these constraints reduces the index of the differential-algebraic equations from 3 to 1, which provides numerical benefits~\cite{Hairer:1996,Campbell:2016,Betts:2010}. To avoid overconstraining the problem, we introduce variables for Lagrange multipliers, $\bar{\lambda}_i$, and velocity corrections, $\bar{\gamma}_i$, at the mesh interval midpoints; the length of these vectors is the number of kinematic constraints. The product of the transpose of the kinematic constraint Jacobian and the velocity corrections, $G(\bar{q}_i, p)^T \bar{\gamma}_i$, yields a correction to the generalized speeds that is perpendicular to the constraint manifold $\phi = 0$. This correction is necessary because the Hermite splines cannot simultaneously satisfy the differential equations in addition to the kinematic constraints. This method for handling kinematic constraints with the Hermite-Simpson transcription was introduced by Posa and colleagues~\cite{Posa:2016}. Although Simbody supports non-holonomic and acceleration-level constraints, Moco only supports holonomic (position-level) constraints.
