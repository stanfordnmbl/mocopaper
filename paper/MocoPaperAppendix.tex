...
The variables $\bar{y}_i$ (including $\bar{q}_i$, $\bar{u}_i$, $\bar{z}_{\mathrm{ex},i}$, $\bar{z}_{\mathrm{im},i}$), $\bar{x}_i$, and $\bar{\zeta}_i$ are associated with the midpoint of mesh interval $i$.

Eq~\ref{hermitesimpsonnlp} includes the first and second time derivatives of the kinematic constraints:

\hermitesimpsonkincon

Including these constraints reduces the index of the differential-algebraic equations from 3 to 1. To prevent the problem from being overconstrained, we introduce variables for Lagrange multipliers, $\bar{\lambda}_i$, and velocity corrections, $\bar{\gamma}_i$, at the mesh interval midpoints. When combined with the transpose of the kinematic constraint Jacobian, $G^T$, to obtain $Lagrange multipliers supply forces $G

Both the Lagrange multipliers and the velocity corrections are perpendicular to the 

This method for handling kinematic constraints with the Hermite-Simpson transcription was introduced by Posa and colleagues~\cite{Posa:2016}. Although Simbody supports non-holonomic and acceleration-level constraints, Moco only supports holonomic (position-level) constraints.

The Hermite cubic spline cannot satisfy both the differential equation
The $G(\bar{q}_i, p)^T \bar{\gamma}_i$ term is a velocity correction that is necessary when enforcing derivatives of kinematic constraints in the nonlinear program. The additional variables $\bar{\gamma}_i$ are velocity corrections (TODO) in the directions of the kinematic constraints (in the same space as the Lagrange multipliers $\lambda$) and prevent the kinematics from being overconstrained. The kinematic constraint Jacobian transpose operator $G(\bar{q}_i, p)^T$ restricts the velocity correction to the tangent plane of the constraint manifold. :
